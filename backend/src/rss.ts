...
const weatherCache: Map<string, { data: any, ts: number }> = new Map();
...
const fetchRealWeather = async (lat: number, lon: number) => {
  ...
  try {
    const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&wind_speed_unit=kmh`, { signal: AbortSignal.timeout(3000) });
    if (res.ok) {
...
const parseRSSItem = (itemText: string, index: number) => {
  try {
...
export const fetchAndProcessRSS = async () => {
  ...
  try {
    const res = await fetch('https://www.hzspa.cz/vyjezdy/rss-aktualni-vyjezdy.php', {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; HZSDashboard/1.0)',
    ...
    const parsedItems = itemMatches.map((item: string, i: number) => parseRSSItem(item, i)).filter((i: any) => i !== null);
    ...
    for (const item of parsedItems) {
      ...
      if (!existing) {
        ...
        await sendPushNotification(eventObj);
      }
    }
