...
export const DispatchCard: React.FC<DispatchCardProps> = ({ dispatch, style, userLocation, onClick }) => {
  ...
  const ageMs = Date.now() - dispatch.time.getTime();
  const isRecent = ageMs < 60 * 60 * 1000;
  // Freshness: 100% = just now, 0% = 12h+ ago
  const freshness = Math.max(0, Math.min(100, 100 - (ageMs / (12 * 60 * 60 * 1000)) * 100));
  ...
  return (
    <div
      ...
      onClick={onClick}
      style={{ ...style, '--card-color': config.color, '--card-bg': config.bg } as React.CSSProperties}
    >
      {isRecent && isLive && <div className="card-live-dot" title="Probíhá" />}
      ...
      <div className="card-location" style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        {distanceStr && <span className="distance-badge"><Navigation size={11} /> {distanceStr}</span>}
      </div>
      ...
      {isLive && (
        <div className="card-freshness">
          <div className="card-freshness-bar" style={{ width: `${freshness}%`, opacity: 0.6 + freshness * 0.004 }} />
        </div>
      )}
    </div>
  );
